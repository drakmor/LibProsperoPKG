...
- Signed outer-PFS files use the hierarchy consumed by `PfsReader`: 12 direct descriptors, one single-indirect table in `ib[0]`, then a double-indirect root in `ib[1]` whose entries address signed leaf tables. With 64-KiB blocks and 36-byte `{SHA3-256, block}` descriptors, one inode addresses 3,314,232 blocks (about 202.3 GiB). Both the in-memory and file-backed writers emit this hierarchy.
...
- For publisher PPR/NAPS, computes FIH `0xB0` as `SHA3-256(naps_pkg_layout.dat)` and FIH `0xA8` as that blob's exact byte length. This formula is byte-exact against `dlc_baseline.pkg`. Standalone finalization without the build artifact falls back to an outer-image hash.
...
- `CompressedPfsFileWriter.WriteCompressed(payload)` output is accepted by the reference decompressor and round-trips byte-for-byte for the covered cases: single chunk, multi-block payloads over 256 KiB, the exact 0x40000 boundary, two internal chunks per block with cross-chunk back-references, per-half stored fallback, and raw/sub literal selection. Its PFSC boundary table now preserves the exact per-half flag byte instead of assuming only `0x06`/`0x26`.
...
- The SI segment (the trailing `sce_suppl` ZIP) is now emitted automatically by the `nwonly` build. `ProsperoPkgBuilder` captures the CNT `playgo-chunk.dat` and block-aligned inner-image size; `ProsperoPackageBuilder` passes them to `ProsperoFihBuilder.BuildFromCnt` through an `siArchiveFactory`. The produced segment carries the four `naps_meta_300/301/302/308.dat` records, the copied `playgo-chunk.dat`, a reproducible `config/<content-id>/playgo-chunk.crc`, and an automatically serialized/encrypted `naps_meta_18.dat`. In the verified Publishing Tools 2.79 APP/AC profile, every `naps_meta_30x` member is the same 48-byte plaintext descriptor within one package: six `u64le` values `0, 0, R, 0x3E9, R, 0x20000`, where `R = innerImageSize - 0x20000`. The value is not asserted to be constant across packages. The library derives the type-18 TLV geometry, `ihsh` SHA3/checksum rows, `rhsh` rolling hashes and exact `obcc`. The `sc2 estimate` PFS-image key is now derived locally as `HMAC-SHA256(EKPFS(primary-id, passcode), pfs-image-seed)`; the primary id defaults to content id but is independently configurable. The built-in `obcc` path then derives `D = HMAC-SHA256(pfs-image-key, LE32(1) || seed)`, AES-XTS-encrypts each physical 64-KiB block with data key `D[16..31]`, tweak key `D[0..15]` and physical block index, then stores little-endian CRC32C. Both KDFs match captured Publishing Tools 2.79 output. Verified APP/AC `nwonly` profiles omit `pfsimage.xml`.
...
- The GP5 project model is parsed and emitted for both root-directory-walked and flat files/folders layouts. It preserves current publisher attributes (`entitlement_key`, `c_date`, PlayGo languages/layers, per-file chunk/content-config/compression), implicit `src_path=dst_path`, explicit empty `<files/>`, and recursive overlay/virtual directories. SDK 2.000 PlayGo and nested-root examples plus a Publishing Tools 2.79 PSAL project were accepted after a C# read/write round trip.
...
- `selftest-large-outer --full <scratch-directory>` performs the complete boundary test. The
  verified run produced a 217,321,504,768-byte encrypted outer PFS from a
  217,201,508,353-byte sparse file, then opened it through
  `ProsperoOuterPfsDecryptReader`; `PfsReader` traversed all 3,314,233 mappings and the test
  checked the real `ib[2]` root/middle/leaf SHA3 chain, final data block and superblock ICV.
  The generated corpus is retained under `J:\libprospero-full-ib2-20260726`.
- AC/AL builds require existing backend-issued `license.dat` and `license.info`. They may be
  ...
  Regression builds now exercise provider-backed GD, AC, and AL containers, including GD
  `license.info` key index 2, AC/AL key index 4, protected extraction, and rejection of malformed
  RIF framing, wrong content id, wrong entitlement key, and truncated `license.info`.
...
- The signed 32-bit inode hierarchy is implemented as depth-1 through depth-5 trees in `ib[0..4]`; file-backed outer creation and `PfsReader` share that model. The complete self-generated `ib[1]` to `ib[2]` boundary now passes at 217.3 GB. A same-size publisher image is still needed only for external byte-order/layout corpus confirmation.
- `naps_meta_300/301/302/308.dat` are reproduced byte-exact by `ProsperoNapsMeta` from the build's block-aligned inner-image size and emitted automatically. The implementation writes one 48-byte descriptor and reuses it verbatim for ids 301, 302 and 308. Fresh extraction tests confirmed four 48-byte files and one common SHA-256 within each package across empty, small and multi-block AC plus a complete SDK APP; the APP descriptor correctly differed from the AC descriptor because `R` differed. This identity claim is scoped to the verified Publishing Tools 2.79 APP/AC `nwonly` profile, not every possible SI profile. `naps_meta_18.dat` is also built and AES-XTS encrypted automatically. Its geometry, file table, SHA3/checksum `ihsh`, rolling `rhsh`, and protected `obcc` are generated locally. The high-level builder derives the separate `sc2 estimate` PFS image key from primary id, passcode, and the effective image seed, so exact `obcc` no longer needs an external key pair. The low-level `ProsperoNapsMeta` API still zero-fills `obcc` when invoked without a key/seed/provider, while a supplied publisher-authored blob remains verbatim.
...
- The high-level builder fills every `pfsimage.xml` digest row from its completed CNT/FIH: `content`,
