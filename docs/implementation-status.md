...
- The SI segment (the trailing `sce_suppl` ZIP) is now emitted automatically by the `nwonly` build. `ProsperoPkgBuilder` captures the CNT `playgo-chunk.dat` and block-aligned inner-image size; `ProsperoPackageBuilder` passes them to `ProsperoFihBuilder.BuildFromCnt` through an `siArchiveFactory`. The produced segment carries the four `naps_meta_300/301/302/308.dat` records, the copied `playgo-chunk.dat`, a reproducible `config/<content-id>/playgo-chunk.crc`, and an automatically serialized/encrypted `naps_meta_18.dat`. The library derives its TLV geometry, `ihsh` SHA3/checksum rows, `rhsh` rolling hashes and exact `obcc`. The built-in `obcc` path derives `D = HMAC-SHA256(pfs-image-key, LE32(1) || seed)`, AES-XTS-encrypts each physical 64-KiB block with data key `D[16..31]`, tweak key `D[0..15]` and physical block index, then stores little-endian CRC32C. It matched all three records in a fresh Publishing Tools 2.79 package. Verified APP/AC `nwonly` profiles omit `pfsimage.xml`.
...
- `PprPfsKrakenTool selftest` includes a full deterministic APP/PPR-NAPS regression. It creates equivalent source trees in opposite host creation order, independently builds both finalized packages, requires byte-for-byte equality, and parses the result as `FullDebug`.
...
- `ProsperoBuildOptions.RequirePublisherCompatibility` (CLI trailing mode `strict`) turns the external-input warnings into pre-build errors. It requires a caller/sidecar metadata signer, a publisher-authored 0x800-byte `PublisherImageKey`, and, for NAPS profiles, either a publisher `naps_meta_18.dat`, an `IProsperoNapsIntegrityProvider`, or the complete `NapsPfsImageKey`/`NapsPfsImageSeed` pair. The inputs can also be loaded from raw `pkg_image_key.bin`, `pfs_image_key.bin` (32 bytes), and `pfs_image_seed.bin` (16 bytes) sidecars. A NAPS CMAC key is not required for the verified Publishing Tools 2.79 debug/AC profile: its config constructor leaves CMAC mode disabled and real packages contain zero eight-byte outer-block tags. `NapsOuterBlockCmacKey` remains an optional input for another profile that explicitly enables keyed CMAC. Only `img_info`/`img_verify` can establish that the supplied signer and package inputs belong to the correct trust profile.
...
- `naps_meta_300/301/302/308.dat` (the 48-byte records) are reproduced byte-exact by `ProsperoNapsMeta` from the build's own inner-image size and emitted in the SI segment automatically. `naps_meta_18.dat` is also built and AES-XTS encrypted automatically. Its geometry, file table, SHA3/checksum `ihsh`, rolling `rhsh`, and protected `obcc` are generated locally. Exact `obcc` requires the package's separate `sc2 estimate` PFS image key/seed pair; without a pair or provider it is intentionally zero-filled, while a supplied publisher-authored blob remains verbatim.
