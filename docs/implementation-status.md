...
- The specialized publisher `ProsperoPs5InnerImageAssembler` now has `BuildToFile` and `BuildFromFsTreeToFile`. The normal package path writes its physical `pfs_image.dat` directly to a temporary file; NAPS CMAC, `obdg`, outer-PFS input, and optional integrity providers consume that file. The in-memory compatibility methods remain byte-identical. Per-file plaintext and compressed payload arrays are still retained because the current Kraken encoder and `ihsh/rhsh` construction consume them.
