...
- The SI segment (the trailing `sce_suppl` ZIP) is now emitted automatically by the `nwonly` build. `ProsperoPkgBuilder` captures the CNT `playgo-chunk.dat` and block-aligned inner-image size; `ProsperoPackageBuilder` passes them to `ProsperoFihBuilder.BuildFromCnt` through an `siArchiveFactory`. The produced segment carries the four `naps_meta_300/301/302/308.dat` records, the copied `playgo-chunk.dat`, a reproducible `config/<content-id>/playgo-chunk.crc`, and an automatically serialized/encrypted `naps_meta_18.dat`. The library derives its TLV geometry, `ihsh` SHA3/checksum rows and `rhsh` rolling hashes; exact `obcc` values still require `IProsperoNapsIntegrityProvider` because their CRC32C input is the publisher's temporary AES-XTS representation. Verified APP/AC `nwonly` profiles omit `pfsimage.xml`.
...
- Publisher artifact creation is also file-backed: `BuildFileBacked` streams NAPS in 256-KiB units, writes and hashes the outer PFS in 64-KiB units, and applies AES-XTS through temporary files with atomic final replacement. The normal `build-pkg` path uses the same file-backed outer writer and copies the result into CNT instead of retaining a second complete outer-image array. The specialized `ProsperoPs5InnerImageAssembler` model itself still retains its assembled inner image and per-file data in memory.
...
- `PprPfsKrakenTool selftest-large-outer` exercises the first double-indirect data block: it builds a 1,833-block (120,127,488-byte) sparse file, decrypts the result, checks `ib[0]` and `ib[1]`, and traverses the complete file through `PfsReader`.
...
- The signed 32-bit inode hierarchy is implemented through `ib[1]` (about 202.3 GiB per file). The meanings of `ib[2..4]`, a possible triple-indirect profile, and any fragmented/extent profile above that size remain unconfirmed.
- `naps_meta_300/301/302/308.dat` (the 48-byte records) are reproduced byte-exact by `ProsperoNapsMeta` from the build's own inner-image size and emitted in the SI segment automatically. `naps_meta_18.dat` is also built and AES-XTS encrypted automatically. Its geometry, file table, SHA3/checksum `ihsh`, and rolling `rhsh` are generated locally; only exact protected `obcc` remains provider-supplied and is zero-filled without a provider.
...
- External `prospero-pub-cmd 2.79 img_info/img_verify` still rejects a from-scratch generated package as an invalid format before NAPS integrity checking. The origin audit additionally corrected true RSA-3072 passcode/mount-image wrapping, 64-KiB CNT body alignment, CNT 0x4A0/0x4B0/0x510 descriptors, two-region PlayGo geometry, FIH inode counts, and the exact FIH 0xB0 NAPS-layout digest formula. Remaining suspects are mandatory AC license entries, exact publisher PlayGo/SI metadata, and the separate NAPS CMAC input. This is an explicit compatibility gap; the internally successful round trip is not reported as RIC success.
