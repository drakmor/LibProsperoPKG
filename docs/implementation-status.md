...
- `ProsperoBuildOptions.RequirePublisherCompatibility` (CLI trailing mode `strict`) turns the external-input warnings into pre-build errors. It requires a caller/sidecar metadata signer and, for NAPS profiles, a 16-byte outer-block CMAC key plus either a publisher `naps_meta_18.dat` or an `IProsperoNapsIntegrityProvider`. This prevents research-key/zero-integrity output from being mistaken for a package already accepted by Publishing Tools; only `img_info`/`img_verify` can establish that the supplied providers belong to the correct trust profile.
