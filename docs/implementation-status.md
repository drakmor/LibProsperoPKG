...
- External `prospero-pub-cmd 2.79 img_info/img_verify` still rejects a from-scratch generated package as an invalid format before NAPS integrity checking. The current audit already corrected the publisher CNT/FIH constants, FIH 0xA8 layout size, body ordering, 0xB80 `ENTRY_KEYS`, 0x800 `IMAGE_KEY`, and 384-byte RSA-3072 header signature. Remaining suspects are the unavailable genuine RSA-3072 entry-key ciphertexts/image-key slots, mandatory AC license entries, and exact publisher PlayGo/SI metadata. This is an explicit compatibility gap; the internally successful round trip is not reported as RIC success.
