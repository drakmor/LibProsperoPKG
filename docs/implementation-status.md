...
- A single outer-PFS inode currently supports 12 direct blocks plus five single-indirect tables (9,112 64-KiB blocks, about 569.5 MiB). Larger `pfs_image.dat` files require confirming and implementing the format's multi-level/extent addressing rather than silently overflowing the inode.
