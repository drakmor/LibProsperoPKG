...
- The legacy profile emits `<chunkinfo>`, `<pfs-image>`, and `<nested-image>` from captured inode trees. The publisher profile currently emits the correct chunk geometry but does not yet synthesize equivalent XML inode trees for the relocated direct-offset/NAPS view. The nested `<metadata>` pseudo-element and per-file `poffset` are intentionally omitted until they can be derived without guessing.
