...
| `ProsperoDirectoryRetailFinalizationProvider` | Imports standard-Retail 0x300/0x180 results exported from an existing package. Binary SHA3-256 request bindings make the material usable only for an exact FIH/CNT rebuild; a mismatch is rejected before output is written. |
...
| `ProsperoPs5InnerImageAssembler` | Specialized publisher `nwonly` data-first inner-image assembler. `Build`/`BuildFromFsTree` return compatibility in-memory images; `BuildToFile`/`BuildFromFsTreeToFile` write the physical image directly to disk. The `FSDir` bridge preserves empty directories; flat callers can supply `ProsperoPs5InnerDirectory` explicitly. Ordinal dirent AFID traversal, block-contained directory entries, automatic sparse slots, empty-file terminal AFIDs, and bootstrap-only physical alignment reproduce the publisher's dense-tree policy. `ProsperoPs5InnerImageResult.OpenImage` abstracts the two result forms and preserves placement/compression metadata for NAPS and SI generation. |
...
| `ProsperoOuterPfsDecryptReader` | Cached random-access plaintext view over an encrypted outer PFS. It applies the per-block Data/Signed/Plaintext sector policy and lets `PfsReader` inspect multi-hundred-GiB images without writing a full decrypted copy. |
...
| `ProsperoOuterPfsBuilder` | PS5 nwonly outer-PFS **structure generator**: assembles the data-first plaintext outer image from its files (`pfs_image.dat`, `naps_pkg_layout.dat`) — inode table with per-block SHA3 hashes, super-root/uroot dirents, the `\x7fFLT` inode_flat_path_table (custom reduced-Keccak path hash), and the signed superblock (+`icv`). `BuildPlaintext`, `Encrypt`, `BuildEncrypted`, `BuildForPackage`, and bounded-memory `BuildForPackageToFile`; the latter accepts `encryptOutput: false` for a file-backed plaintext diagnostic carrier. Signed files use 12 direct blocks and recursive signed maps in `ib[0..4]`; `ib[2]` removes the former 202.3-GiB ceiling. Types: `ProsperoOuterFile`, `ProsperoOuterFileSource`, `ProsperoOuterPfsBuildParameters`, `ProsperoOuterPfsBuildResult`, `ProsperoOuterPackageFileResult`. |
| `ProsperoPublisherPprBuilder` | Publisher artifact pipeline from a source folder through relocated direct-offset PPR-PFS (`mode 0x18`, superblock at logical 0x400000), NAPS, data-first outer PFS and optional final AES-XTS. `Build` is the compatibility in-memory API; `BuildFileBacked` streams the logical/NAPS/outer layers to files and returns `ProsperoPublisherPprFileBuildResult`. `ProsperoPublisherPprBuildOptions.EncryptOuterPfs=false` emits a plaintext outer PFS for analysis; normal PKG creation always uses the encrypted form. Both return publisher `imagedigs.dat` metadata and the logical digest and validate the reverse path. |
...
| `CompressedPfsFileWriter` | Produce a PFSv3 `PFSC` container. `WriteCompressed(payload, level, blockSize, useHuffmanArrays=true)` (Kraken with default-on Huffman entropy arrays, automatic raw/sub literals, and independent stored fallback for each 128-KiB half) / `WriteStored(payload)`. |
| `ProsperoNapsImage` | Resolves publisher NAPS fidx/U2C/CblockInfo tables into `ProsperoNapsPlan`/`ProsperoNapsSpan` and reconstructs the complete logical inner PPR-PFS stream with `BuildPlan` and `Decompress`. `Pack` is the inverse monotonic Kraken/stored writer; it generates all observed per-half CBI modes (`5/4`, `7/6`, `7/4`, `5/6`, `1/1`, `1/4`, `1/6`, `5/1`, `7/1`, `5/0`, `7/0`, `1/0`) from the encoder result rather than forcing `5/4`. Modes ending in `/0` mean an absent odd half; a present stored odd tail is `/1`. The publisher layout generator also emits the conditional final `dataEnd` FIDX boundary and keeps semantic RUN records out of CBI slot 15. Its stream overload processes one 256-KiB block and returns `ProsperoNapsFileBuildResult`. `ComputeOuterBlockDigest` implements the optional SHA3/reverse/AES-CMAC tag chain. |
| `ProsperoAfidMap` | Extracts exact path-to-AFID assignments from a decrypted PPR-PFS image and saves/loads the stable TSV sidecar. Supplying the result as `ProsperoBuildOptions.PublisherAfidAssignments` (or CLI `build-pkg ... afid=<map.tsv>`) preserves sparse `-1` slots, 256-KiB zero extents, FIDX type `0x40` boundaries, NAPS dedup records, and SI integrity geometry. |
...
| `Oodle.KrakenDecoder` | Internal newLZ (Kraken) decoder: raw + Huffman literal/cmd/offset/length arrays, post-seed excess framing with length escapes, raw and sub/delta literal models, directly stored or entropy-coded halves, mixed stored/newLZ blocks, multi-chunk and multi-block. |
...
- **`Gp5Project`** — the GP5 document model, with both the "normal" (`rootdir`-walked) and
  ...
  overlay/virtual directories. The package builder consumes both layouts directly, including
  normal-rootdir and global/directory/file exclusion masks.
