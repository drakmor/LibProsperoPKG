...
| `ProsperoOuterPfsBuilder` | PS5 nwonly outer-PFS **structure generator**: assembles the data-first plaintext outer image from its files (`pfs_image.dat`, `naps_pkg_layout.dat`) — inode table with per-block SHA3 hashes, super-root/uroot dirents, the `\x7fFLT` inode_flat_path_table (custom reduced-Keccak path hash), and the signed superblock (+`icv`). `BuildPlaintext`, `Encrypt`, `BuildEncrypted`. The current inode writer supports 12 direct plus five single-indirect block tables and rejects a larger file explicitly. Types: `ProsperoOuterFile`, `ProsperoOuterPfsBuildParameters`, `ProsperoOuterPfsBuildResult`. |
...
| `ProsperoNapsImage` | Resolves publisher NAPS fidx/U2C/CblockInfo tables into `ProsperoNapsPlan`/`ProsperoNapsSpan` and reconstructs the complete logical inner PPR-PFS stream with `BuildPlan` and `Decompress`. `Pack` is the inverse monotonic Kraken/stored writer; `ComputeOuterBlockDigest` implements the optional SHA3/reverse/AES-CMAC tag chain. |
